...
export default function Account() {
  const { user, membership, hasPaidAccess, loading, openAuth, signOut, refreshMembership } = useCommerce();
  ...
  return (
    <div className="commercial-page"><SiteHeader /><main className="account-page">
      ...
      <section className="membership-panel"><div><span className="panel-label">当前套餐</span><h2>{hasPaidAccess && plan ? `${plan.name}会员` : '免费账户'}</h2><p>{hasPaidAccess && membership?.lifetime ? '永久权益，无自动续费' : hasPaidAccess && membership?.expiresAt ? `有效期至 ${new Date(membership.expiresAt).toLocaleDateString('zh-CN')}` : '浏览案例免费，导出视频需要开通会员。'}</p></div><span className={`membership-status${hasPaidAccess ? ' active' : ''}`}><CheckCircle2 size={14} />{hasPaidAccess ? '已生效' : '未开通'}</span></section>
      <div className="account-actions">{membership ? <button className="account-action" onClick={() => void openPortal()} disabled={portalLoading}><CreditCard size={20} /><span><strong>{portalLoading ? '正在打开' : '管理订阅与账单'}</strong><small>修改付款方式、查看账单或取消续费</small></span></button> : <Link to="/pricing" className="account-action"><CreditCard size={20} /><span><strong>开通会员</strong><small>选择月度、季度、年度或永久版</small></span></Link>}<button className="account-action" onClick={() => void signOut()}><LogOut size={20} /><span><strong>退出登录</strong><small>退出当前账户</small></span></button></div>
